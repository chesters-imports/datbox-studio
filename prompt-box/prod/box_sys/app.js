...
(function () {
  ...
  // Expose for index.html dynamic loader (preferred). Guard double-boot.
  window.__promptBoot = function () {
    if (window.__promptBooted) return Promise.resolve();
    window.__promptBooted = true;
    return boot().catch(err);
  };

  // Self-start if page used classic script tags (no dynamic loader)
  if (!window.__promptUseLoader) {
